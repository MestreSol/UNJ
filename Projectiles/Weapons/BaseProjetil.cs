...
using UNJ.Items.Swords;
using UNJ.Dusts;

namespace UNJ.Projectiles.Weapons
{
    class BaseProjetil : ModProjectile
    {
		...
		public override void Kill(int timeLeft)
		{
			...
			if (projectile.owner == Main.myPlayer)
			{
				...
				Main.rand.NextBool(18)
					? Item.NewItem(projectile.getRect(), ModContent.ItemType<BaseSword>())
					: 0;
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
		{
			...
			target.AddBuff(ModContent.BuffType<Buffs.BaseBuff>(), 900); // Adds the ExampleJavelin debuff for a very small DoT
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is BaseProjetil javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
		...
		private void NormalAI()
		{
			...
			{
				Dust dust = Dust.NewDustDirect(projectile.position, projectile.height, projectile.width, ModContent.DustType<BaseDust>(),
